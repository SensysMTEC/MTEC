...
def run_mtec(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,ED_cost,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,output_lookup,in_out_history,input_lookup):
	pf_mtec_av=[]
	service_time_mtec=[]
	service_time_mtec_norm=[]
		...
		if time not in task_time:
			...
			for stage in vert_stage:			# go through each stage in the dag
				...
				for each_task in vert_stage[stage]:			#go through each task in each stage
					...
					normalized_cost = ED_cost[ED_pred][0]*t_pred_norm
					#weighed decision
					weighted_decision = weight_alpha*t_pred_norm+ weight_beta*ProbF + (1 - weight_alpha - weight_beta)*normalized_cost
					replication = 0
					while ProbF > pF_thrs and replication < num_rep and fail_prev_queue:		# while the pf > pf_thre and replication < rep_num
						...
						new_norm_cost = normalized_cost + t_next_pred_norm*ED_cost[t_next_ed][0] 
						#print("new_ProbF: "+str(new_ProbF))
						#print("t_next_pred_norm: {}".format(t_next_pred_norm))
						if  weight_alpha*t_next_pred_norm + weight_beta*new_ProbF + (1 - weight_alpha - weight_beta)*new_norm_cost < weighted_decision:
							#print("replicating...")
							normalized_cost = new_norm_cost
							ProbF=new_ProbF
							weighted_decision = weight_alpha*t_next_pred_norm+(1-weight_alpha)*ProbF
							replication+=1
							...
							backtrack_dic[t_next_ed][task].append([k+i,t_next_pred,k+i+t_next_pred]) ## newly added
			...
			for each_key in allocation.keys():
				load_dic[int(allocation[each_key][0])]+=1
			...
			service_time_mtec.append(i)
			service_time_mtec_norm.append(i_norm)
			k=k+1
			#print(allocation)
			pf_mtec_av.append(tmp_pf_dic[task_types-1])
		if instance_count >= len(task_time):
			with open('load.json','w') as outfile:
				json.dump(load_dic,outfile)
			break
	end_orch = timer.time()
	#print(f"average orchestration time: {(end_orch-start_orch)/len(task_time)}")
	average_service_time_mtec = sum(service_time_mtec)/len(task_time)
	average_service_time_mtec_norm = sum(service_time_mtec_norm)/len(task_time)
	service_time_mtec_x = []
	time_x = []
	load_ed = [0 for i in range(sim_time) for j in range(num_edge)]
	for each in range(0,sim_time):
		if each in task_time:
			service_time_mtec_x.append(service_time_mtec.pop(0))
			time_x.append(each)

	for i in range(num_edge):
		for j in range(task_types):
			load_ed[i] = np.add(load_ed[i],ED_tasks[j][i])
	print(f"average service time: {average_service_time_mtec}")
	print(f"average pf:{sum(pf_mtec_av)/100}")
	print(f"total cost: {total_cost/100}")
	return time_x, average_service_time_mtec, service_time_mtec_x, pf_mtec_av, load_ed, dispatcher_dic
...
def run_petrel(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,output_lookup):
	...
	print(f"average time:{average_service_time_petrel}")
	print(f"average pf: {sum(pf_petrel_av)/100}")
...
def run_lavea(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,output_lookup):	
	...
	print(f"average time:{average_service_time_lavea}")
	print(f"average pf: {sum(pf_lavea_av)/100}")
...
def run_round_robin(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,output_lookup):
	...
	print(f"average time:{average_service_time_rr}")
	print(f"average pf: {sum(pf_rr_av)/100}")
...
def run_random(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,output_lookup):
	...
	print(f"average time:{average_service_time_rd}")
	print(f"average pf: {sum(pf_rd_av)/100}")
...
def run_lats(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list, ed_cpu_regression,ed_latency_regression,output_lookup):
	...
	print(f"average time:{average_service_time_lats}")
	print(f"average pf: {sum(pf_lats_av)/100}")
...
def run_dcc(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,task_dict,dependency_dic,pf_ed,pf_ed_tk,task_file_dic,app_directory,inputfile_dic,socket_list,task_class_dict, cluster_dict, num_clusters,output_lookup):
						...
						for cluster_num in range(1,num_clusters):
							for each_device in cluster_dict[cluster_num]:
								...
								if predict_time <= t_pred:
									...
									ED_pred = each_device
									#tmp_ed.append((ED_pred,t_pred))
					# tmp_ed.sort(key=lambda x: x[1])
					# tmp_rank=[]
					# for each_pred in tmp_ed:
					# 	if each_pred[1]==tmp_ed[0][1]:
					# 		tmp_rank.append(each_pred)
					# #print(tmp_rank)
					# ED_pred,t_pred=tmp_rank[random.randint(0,len(tmp_rank)-1)]
	...
	for each in range(0,sim_time):
		if each in task_time:
			...
			time_x_dcc.append(each)
			
	for i in range(num_edge):
		for j in range(task_types):
			load_ed_dcc[i] = np.add(load_ed_dcc[i],ED_tasks[j][i])
	print(f"average time:{average_service_time_dcc}")
	print(f"average pf: {sum(pf_dcc_av)/100}")
...
if __name__ =='__main__':
	...
	parser.add_argument('--sch', type=str, nargs="?", const="mtec", default="mtec",help='joint optimization parameter alpha')
	parser.add_argument('--bt', type=float, nargs="?", const=0.5, default=0.5,help='joint optimization parameter alpha')
	...
	for edge_index in range(num_edge_max-1,num_edge_max):
		...
		ED_cost = np.array(pd.read_excel(EDmc_file,engine="openpyxl",sheet_name="ed_cost",skiprows=0, nrows= num_edge))
	...
	#		lam2 = np.concatenate((lam2,lam2))
		...
		for i in range(num_edge) :
			edge_info[i]={"total": 10000, "available": 4000}
		if args.sch == "mtec":
			time_x, average_service_time_mtec, service_time_mtec_x, pf_mtec_av,load_ed,dispatcher_dic=run_mtec(task_time,num_edge,task_types,vert_stage,ED_m,ED_c,ED_cost,task_dict,dependency_dic,pf_ed,pf_ed_tk, task_file_dic,app_directory,inputfile_dic, global_var.socket_list,output_lookup,global_var.in_out_history,input_lookup)
		if args.sch == "petrel":
